//Implementacion de la regla de la mayoria pero se evaluan todos los vecionos (similar a el movimento de una reina)
...
function draw() { //establecemos el tablero
    ...
    for (let i = 0; i < columnas; i++) {
        for (let j = 0; j < filas; j++) {
            ...
            if (vecinos > 4) { //priemra regla si una celula esta muerta pero tiene 3 vecinos vivos revive
                sig[i][j] = 1;
            } else if (vecinos < 4) { //si esta viva pero tiene menos de 3 vecinos o mas de 3 se muere
                sig[i][j] = 0;
            } else {
